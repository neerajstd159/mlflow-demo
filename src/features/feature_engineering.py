...
def apply_tfidf(train_df: pd.DataFrame, test_df: pd.DataFrame, max_features: int) -> tuple:
    """Apply tfidf vectorizer to data"""
    try:
...
def save_data(train_df: pd.DataFrame, test_df: pd.DataFrame, data_path: str) -> None:
    ...
    try:
        ...
        train_df.to_csv(os.path.join(processed_data_path, 'train_tfidf.csv'), index=False)
        test_df.to_csv(os.path.join(processed_data_path, 'test_tfidf.csv'), index=False)
...
def main():
    try:
        ...
        # apply tfidf
        train_df, test_df = apply_tfidf(train_df, test_df, max_features)
